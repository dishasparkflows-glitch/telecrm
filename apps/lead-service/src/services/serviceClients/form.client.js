...
const getFormsBulk = async (tenantId, ids) => {
    if (!tenantId || !ids || ids.length === 0) return [];
    try {
        const queryParams = new URLSearchParams({ ids: ids.join(',') });
        const path = `/api/forms/internal/bulk?${queryParams.toString()}`;
        ...
        const response = await axios.get(
            `${env.SERVICES.FORM}${path}`,
            { headers, timeout: 5000 }
        );
        return response.data?.data || [];
    } catch (error) {
        console.warn(`Bulk form lookup failed: ${error.message}`);
        return [];
    }
};

module.exports = { getFormsBulk };
